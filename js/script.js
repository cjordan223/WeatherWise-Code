...
async function displayLocation() {
    ...
    let url = `https://api.weatherapi.com/v1/current.json?key=KEY&q=${locationValue}&aqi=no`;
    ...
    try {
        ...
        let furl = `https://api.weatherapi.com/v1/forecast.json?key=KEY&q=${locationValue}&days=7`;
...
console.log("Temperature:", data.current.temp_f + " °F");
document.querySelector("#dispTemp").innerHTML = data.current.temp_f + " °F";

console.log("Condition:", data.current.condition.text + ` <img src="https:${data.current.condition.icon}" alt="condition">`);
document.querySelector("#dispCond").innerHTML = data.current.condition.text + ` <img src="https:${data.current.condition.icon}" alt="condition">`;

// Check the styles after setting the innerHTML
console.log("Background Style:", document.querySelector("#currentBG").style.backgroundImage);
console.log("Text Color Style:", document.querySelector("#currentBG").style.color);
